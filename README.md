...
2. A computer model of how neurons send signals
3. A physical simulation of a fruit fly body
...
If a simple model of the neurons + the real wiring + a realistic body can produce recognizable behavior, it supports the idea that structure carries a large amount of function. It also creates a useful test system: we can turn neurons on or off in the model and immediately see the effect on behavior — something that is slow and difficult in living animals.

The work is also practical. Building the full pipeline requires real skill in computational neuroscience, biomechanics simulation, and careful systems integration. Documenting the process openly turns the project into both a research contribution and a clear record of ability.
...
1. **The environment**  
   A simple virtual world that can contain food, surfaces, and other stimuli.

2. **Sensory input**  
   Events in the virtual world (touching sugar, dust on the antennae, etc.) are turned into activity in the corresponding sensory neurons of the connectome model.

3. **The brain model**  
   A network of simplified neurons built directly from the real connectome. When sensory neurons become active, signals spread through the network according to the biological connections.

4. **The body**  
   A detailed physical model of a fruit fly. Activity in certain output neurons of the brain model is used to control the fly’s legs, head, and feeding structures.
...
Everything used in this project is free and publicly available.
...
No paid software or cloud services are required to run the core project.
...
Bates, A.S., Phelps, J.S., Kim, M. et al. (2026). Distributed control circuits across a brain-and-cord connectome. *Nature*, 656, 957–970.  
